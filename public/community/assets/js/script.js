...
function toggleNotification() {
    var open = $("#dropdownStatus").val();
    if(open == 1) {
        $("#dropdownStatus").val(0);
...
function enable(id) {
    var isOpen = $("#isOpen_"+id).val();
    if(isOpen == 0) {
        $('#ldr-comments-'+id).show();
        $("#isOpen_"+id).val(1)
        $('#comment-section-'+id+' .main-comment').hide();
        setTimeout(function() {
            $('#ldr-comments-'+id).hide();
            $('#comment-section-'+id+' .main-comment').show();
        }, 2000);
    }else {
        $("#isOpen_"+id).val(0)
        $('#comment-section-'+id+' .main-comment').hide();
