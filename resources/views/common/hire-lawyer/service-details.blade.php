        ...
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-md-12 text-center margin-seven-bottom">
                    <h6 class="alt-font text-extra-dark-gray font-weight-500">Buy Services From</h6>
                </div>
            </div>
            <div class="row row-cols-1 row-cols-lg-4 row-cols-sm-2 interactive-banners-style-04">
                @foreach($serviceLawyers as $k => $lawyer)
                    ...
                    <div class="col text-center interactive-banners-content sm-text-center md-margin-30px-bottom xs-margin-15px-bottom wow animate__zoomIn" data-wow-delay="0.2s">
                        <div class="position-relative padding-4-rem-tb bg-white box-shadow-large overflow-hidden">
                            <img src="/storage/{{$lawyer->getProfilePic($lawyer->lawyer_id)}}" class="w-10 margin-35px-bottom sm-w-100px" alt=""/>
                            <span class="alt-font text-extra-dark-gray d-block font-weight-500 line-height-22px">{{$lawyer->user->name}}</span>
                            <span class="d-block">{{$lawyer->getEmirate($lawyer->lawyer_id)}}</span>
                            <span class="d-block text-black"><b>{{$lawyer->platform_fee + $lawyer->lawyer_fee}} USD</b></span>
                            
                            <!-- <button type="submit" class="btn btn-medium bg-extra-dark-gray d-block text-white section-link">Hire Now</button> -->
                            @if(auth()->user())
                                <a href="#free-advice-form-{{$k}}" class="btn btn-medium bg-extra-dark-gray d-block text-white section-link popup-with-form" onclick="requestForQuotes('{{$lawyer->id}}')">Hire Now</a>
                            @else 
                                <a href="#login-form" class="btn btn-medium bg-extra-dark-gray d-block text-white section-link popup-with-form">Hire Now</a>
                            @endif
                        </div>
                    </div>
                                            ...
                                            <div class="tab-content">
                                                <div class="tab-pane fade show active" id="nav-tab-card">
                                                    ...
                                                    <form role="form" method="POST" id="paymentForm" action="{{ route('stripe.payment') }}">
                                                        ...
                                                        <input type="hidden" name="amount" value="{{$lawyer->platform_fee + $lawyer->lawyer_fee}}">                                                   
...
@section('script')
    <script>
        function requestForQuotes(id) {
            $.ajax({
                method:"post",
                url: "/request-for-quotes/store",
                data: {
                    "_token": "{{ csrf_token() }}",
                    "id" : id
                },
